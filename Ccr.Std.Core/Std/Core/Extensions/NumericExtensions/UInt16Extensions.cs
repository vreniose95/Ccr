...
namespace Ccr.Std.Core.Extensions.NumericExtensions
{
	...
	public static class UInt16Extensions
	{
		...
		///		<see cref="UInt16"/> subject with the provided <paramref name="value"/> parameter, and returns 
		///		the largest numeric <see cref="UInt16"/> value of the two.
		/// </summary>
		/// <param name="this">
		///		The subject <see cref="UInt16"/> to perform the comparison upon.
		/// </param>
		/// <param name="value">
		///		The value of type <see cref="UInt16"/> in which to perform the comparison against the extension 
		///		method's subject, the <paramref name="this"/> parameter.
		/// </param>
		/// <returns>
		///		Compares the extention method's <see cref="UInt16"/> subject and the <paramref name="value"/> 
		///		parameter, and returns the largest numeric <see cref="UInt16"/> value of the two. 
		/// </returns>
		public static UInt16 Smallest(
			this UInt16 @this,
			UInt16 value)
		{
			return @this < value
		...
		/// <summary>
		///		Extension method that performs a transformation on the <see cref="UInt16"/> subject using
		...
		/// </summary>
		/// <param name="this">
		///		The subject <see cref="UInt16"/> to perform the linear map range re-mapping upon.
		/// </param>
		/// <param name="startRange">
		///		An instance of the type <see cref="UInt16Range"/>, describing a range of numeric values in 
		...
		///	</exception>
		/// <returns>
		///		A <see cref="UInt16"/> value that has been linearly mapped to the <paramref name="startRange"/>
		///		parameter and re-mapped to the <paramref name="endRange"/> parameter.
		/// </returns>
		public static UInt16 LinearMap(
			this UInt16 @this,
			[NotNull] UInt16Range startRange,
			[NotNull] UInt16Range endRange)
		{
		...
		///		to be called on a <see cref="UInt16"/> subject with the range and exclusivity passed as a 
		///		parameter, rather than on the <see cref="IntegralRangeBase{TIntegralType}"/> object 
		///		with a <see cref="UInt16"/> parameter.
		/// </summary>
		/// <param name="this">
		///		The subject <see cref="UInt16"/> value in which to check against the <paramref name="range"/>
		...
		///	</exception>
		/// <returns>
		///		A <see cref="UInt16"/> value indicating whether or not the <paramref name="this"/> subject
		///		is within the provided <paramref cref="range"/> parameter, taking into account the 
		///		<see cref="EndpointExclusivity"/> mode via the <paramref name="exclusivity"/> parameter.
		...
		///	</exception>
		/// <returns>
		///		A <see cref="UInt16"/> value that is the <paramref name="this"/> subject value adjusted to
		...
		/// </returns>
		public static UInt16 Constrain(
			this UInt16 @this,
			[NotNull] UInt16Range range)
		{
			range.IsNotNull(nameof(range));
	  ...
	  public static void ThrowIfWithin(
	    [AssertionCondition(IS_NOT_NULL)] this UInt16 @this,
	    [NotNull] UInt16Range range,
