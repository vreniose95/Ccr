...
{
	[XamlSetMarkupExtension("XamlSetMarkupExtension")]
	public class RippleMouseTrackingService
		: HostedElement<FrameworkElement>
	{
		private static readonly Type _type = typeof(RippleMouseTrackingService);
		  ...
		  [NotNull] string eventName)
		{
			...
			_eventHandlerMethodInfo = _type.GetMethod(
				"OnEventImpl",
			...
			eventInfo.AddEventHandler(sourceObject, handlerDelegate);
		}
		...
		private static bool IsValidEvent(EventInfo eventInfo)
		{
			...
			if (parameters.Length == 2)
				return typeof(EventArgs)
					.IsAssignableFrom(parameters[1].ParameterType);
		...
		public static void XamlSetMarkupExtension(
			...
			[NotNull] XamlSetMarkupExtensionEventArgs eventArgs)
		{
			...
			var binding = eventArgs.Value.As<Binding>();
			...
			var ancestor = grandParentInstance.FindParent(relativeSource.AncestorType);
			memberInvoker.SetValue(targetObject, ancestor);
